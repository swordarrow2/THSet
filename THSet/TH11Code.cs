...
namespace THSet {
    class TH11Code:THCode {
        ...
        public override void setMemoryTool(MemoryTool m) {
            ...
            write(0x004327F0,new byte[] { 0xE9,0x1D,0xF5,0xFF,0xFF,0x90 });
            write(0x00431D12,new byte[] { 0x89,0x15,0x18,0x57,0x4A,0x00,0xEB,0x6B });
            write(0x00431D85,new byte[] { 0xFF,0x05,0x80,0x5E,0x4A,0x00,0xE9,0x66,0x0A,0x00,0x00 });
            write(0x004A5E80,0);
            //主动
            write(0x004311F1,new byte[] { 0xE9,0x52,0xF0,0xFF,0xFF,0x90 });
            write(0x00430248,new byte[] { 0x29,0x15,0xE8,0x56,0x4A,0x00,0xEB,0x12 });
            write(0x00430262,new byte[] { 0xFF,0x05,0x84,0x5E,0x4A,0x00,0xE9,0x8A,0x0F,0x00,0x00 });

            //决死
            write(0x0043129D,new byte[] { 0xE9,0xA0,0x09,0x00,0x00,0x90 });
            write(0x00431C42,new byte[] { 0x29,0x05,0xE8,0x56,0x4A,0x00,0xFF,0x05,0x84,0x5E,0x4A,0x00,0xEB,0x07 });
            write(0x00431C57,new byte[] { 0xE9,0x47,0xF6,0xFF,0xFF });
        ...
        public override void setFPS(int i) {
            if(write(0x71CD8ABC,i)==0) {
                ...
                write(0x0044647C,new byte[] { 0xDD,0x05,0xE6,0x61,0x44,0x00 });
            }
        }
