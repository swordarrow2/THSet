...
namespace THSet {
    class TH10Code:THCode {
        ...
        public override void setMemoryTool(MemoryTool m) {
            ...
            write(0x004750E0,0);
            //决死
            write(0x00425C43,new byte[] { 0xE9,0x59,0xF2,0xFF,0xFF,0x90,0x90,0x90 });
            write(0x00424EA1,new byte[] { 0x66,0x81,0x05,0x48,0x4C,0x47,0x00,0xEC,0xFF,0xEB,0x07 });
            write(0x00424EB3,new byte[] { 0xFF,0x05,0xE4,0x50,0x47,0x00,0xE9,0x8D,0x0D,0x00,0x00 });
            //主动
            write(0x004259D4,new byte[] { 0xE9,0x7A,0x0C,0x00,0x00,0x90,0x90,0x90 });
            write(0x00426653,new byte[] { 0x66,0x81,0x05,0x48,0x4C,0x47,0x00,0xEC,0xFF,0xEB,0xA7 });
            write(0x00426605,new byte[] { 0xFF,0x05,0xE4,0x50,0x47,0x00,0xE9,0x3B,0xF6,0xFF,0xFF });
